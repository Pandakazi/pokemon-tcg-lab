...
test('Has Ability is URL-backed, resets page, and restores with independent Pokémon state',async()=>{
 ...
 expect(window.location.search).toContain('ability=Yes');expect(window.location.search).not.toContain('page=7')
 await waitFor(()=>expect(fetch).toHaveBeenLastCalledWith(expect.stringContaining('pokemon_types=Water&stages=Basic&ability=Yes'),expect.anything()))
 fireEvent.click(screen.getByRole('button',{name:'Energy'}))
 fireEvent.click(screen.getByRole('button',{name:'Pokémon'}))
 expect(screen.getByLabelText('Yes')).toBeChecked()
 fireEvent.click(screen.getByLabelText('Yes'))
 expect(new URLSearchParams(window.location.search).has('ability')).toBe(false)
})

test('Ability Yes/No is between Stage and Regulation and ORs in the shared URL',async()=>{
 window.history.replaceState({},'','/?category=Pokemon&stages=Basic&pokemon_types=Water&has_ability=true')
 const fetch=vi.fn().mockResolvedValue({ok:true,json:async()=>({...result(),filter_options:[{parameter:'stages',label:'Stage',values:['Basic']},{parameter:'regulation_marks',label:'Regulation mark',values:['H']}]})})
 vi.stubGlobal('fetch',fetch);render(<App/>);await screen.findByText('sv02-097')
 expect([...document.querySelectorAll('legend')].map(e=>e.textContent)).toEqual(['Stage','Ability','Regulation mark'])
 expect(screen.getByLabelText('Yes')).toBeChecked()
 fireEvent.click(screen.getByLabelText('No'))
 expect(new URLSearchParams(window.location.search).getAll('ability')).toEqual(['Yes','No'])
 expect(window.location.search).not.toContain('has_ability')
 fireEvent.click(screen.getByLabelText('Yes'))
 await waitFor(()=>expect(fetch).toHaveBeenLastCalledWith(expect.stringContaining('stages=Basic&pokemon_types=Water&ability=No'),expect.anything()))
 fireEvent.click(screen.getByLabelText('No'))
 expect(new URLSearchParams(window.location.search).has('ability')).toBe(false)
})
