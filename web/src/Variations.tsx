...
import { loadVariations, savePreference, withOwnership, type Card, type Ownership, type VariationPage } from './api'
...
export function Variations({id,library=false,onSelected,onChange,revision=0}:{id:string;library?:boolean;onSelected?:()=>void;onChange?:(id:string,o:Ownership)=>void;revision?:number}) {
 ...
 function changed(id:string,o:Ownership) {
  setData(old=>old?{...old,cards:old.cards.map(c=>withOwnership(c,id,o))}:old);onChange?.(id,o)
 }
 return <section aria-label={library?'Library artwork choices':'Card variations'}>
  ...
  <div className="variation-grid">
   {data?.cards.map(card=><article className="variation-card" key={`${card.id}:${card.ownership!.variant}`} data-variation={`${card.id}:${card.ownership!.variant}`}>
    {library?<button disabled={busy} onClick={()=>choose(card)} aria-label={`Use ${card.id} ${card.ownership!.variant} in Library`}><CardImage card={card}/></button>:
     <MagnifiedArt card={card}><Link to={`/cards/${encodeURIComponent(card.id)}?variant=${encodeURIComponent(card.ownership!.variant)}`} state={{library:location.state?.library||'/'}} aria-label={`Inspect ${card.id} ${card.ownership!.variant}`}><CardImage card={card}/></Link></MagnifiedArt>}
    <strong>{card.name}</strong><p>{card.set.name || card.set.id} · {card.localId}</p><code>{card.id}</code><p>{card.ownership!.variant}</p>
