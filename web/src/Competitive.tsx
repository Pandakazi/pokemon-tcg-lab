...
export type Research = {
 ...
 associated_cards:{functional_id:string;name:string;decks:number;cooccurrence_percent:number;field_percent:number;lift:number}[];association_status:string
 trend:{window:Window;available:boolean;points:Point[]}[];parser_version:string
...
export function CompetitivePopup({id,name,hover}:{id:string;name:string;hover:ReturnType<typeof useCompetitiveHover>}) {
 ...
 return createPortal(<aside role="dialog" aria-label={`${name} competitive preview`} className="competitive-popup" style={hover.position} onPointerEnter={hover.keep} onPointerLeave={hover.leave} onFocus={hover.keep} onBlur={hover.leave} onKeyDown={e=>{if(e.key==='Escape')hover.close()}}>
  ...
  {data&&<><dl><dt>Usage</dt><dd>{pct(data.usage_percent)}</dd><dt>Average copies</dt><dd>{data.average_copies??'Unavailable'}</dd></dl>
...
export function UsageTrend({series}:{series:Research['trend']}) {
 const [hidden,setHidden]=useState<Window[]>([]),[point,setPoint]=useState<string|null>(null)
 const dates=[...new Set(series.flatMap(s=>s.points.map(p=>p.date)))].sort()
 const min=Date.parse(dates[0]||'2000-01-01'),max=Date.parse(dates.at(-1)||'2000-01-01')
 ...
 {series.filter(s=>s.available&&!hidden.includes(s.window)).map((s,i)=>{let pen=false;const path=s.points.map(p=>{if(p.usage_percent===null){pen=false;return ''}const command=pen?'L':'M';pen=true;return `${command}${x(p.date)},${y(p.usage_percent)}`}).join(' ');return <g key={s.window} data-series={s.window}><path d={path} fill="none" stroke={colors[s.window]} strokeWidth="2.5" strokeDasharray={i===0?undefined:i===1?'8 3':i===2?'3 3':'12 3 3 3'}/>{s.points.filter(p=>p.usage_percent!==null).map(p=><circle key={p.date} cx={x(p.date)} cy={y(p.usage_percent!)} r="5" fill={colors[s.window]} tabIndex={0} onMouseEnter={()=>setPoint(p.date)} onFocus={()=>setPoint(p.date)} aria-label={`${label(s.window)} ${p.date}: ${p.usage_percent}% of ${p.sample_size} eligible decklists`}><title>{label(s.window)} · {p.date}: {p.usage_percent}% · {p.included_decks}/{p.sample_size} eligible decklists</title></circle>)}</g>})}
 <text x="55" y="245" fill="currentColor" fontSize="12">{dates[0]}</text><text x="755" y="245" textAnchor="end" fill="currentColor" fontSize="12">{dates.at(-1)}</text>
 </svg>}
 {point&&<div className="trend-point" aria-live="polite">{point}{series.filter(s=>!hidden.includes(s.window)).map(s=>{const p=s.points.find(p=>p.date===point);return <span key={s.window}>{label(s.window)}: {p?.usage_percent===null||p?.usage_percent===undefined?'Unavailable':`${p.usage_percent}%`} · {p?.sample_size??0} eligible decklists</span>})}</div>}
...
export function CompetitiveDashboard({id}:{id:string}) {
 ...
 {data&&<>
  <section><h3>Competitive Overview</h3><dl><dt>Usage</dt><dd>{pct(data.usage_percent)}</dd><dt>Average copies in decks using card</dt><dd>{data.average_copies??'Unavailable'}</dd><dt>Decks using card</dt><dd>{data.included_decks}</dd><dt>Eligible sample</dt><dd>{data.sample_size}</dd></dl></section>
  ...
  <section><h3>Archetypes</h3><p>Where played: share of decks containing this card. Prevalence: share within the archetype; at least 100 eligible archetype decklists required.</p>
   {!data.archetypes.length?<p>No archetype observations.</p>:<div className="research-table"><table><thead><tr><th>Archetype</th><th>Where played</th><th>Prevalence within archetype</th><th>Eligible decks</th></tr></thead><tbody>{data.archetypes.map(a=><tr key={a.id}><th>{a.name}</th><td>{pct(a.share_percent)}</td><td>{a.status==='insufficient_sample'?`Insufficient sample — ${a.eligible_decks} decklists`:a.status==='unclassified'?'Unclassified':pct(a.prevalence_percent)}</td><td>{a.eligible_decks}</td></tr>)}</tbody></table></div>}</section>
  <section><h3>Usage Trend</h3><UsageTrend series={data.trend}/></section>
